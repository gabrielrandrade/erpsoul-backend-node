...
exports.findByEmail = async (email) => {
    ...
    const [rows] = await db.query(`SELECT id_usuario, senha, hash FROM tb_usuario WHERE email = ?`, [email]);
...
exports.updatePassword = async (cryptPass, cryptHash, id_usuario) => {
    const db = await connectDB();
    const [rows] = await db.query(`UPDATE tb_usuario SET senha = ?, hash = ? WHERE id_usuario = ?`, [cryptPass, cryptHash, id_usuario]);
    return rows;
}